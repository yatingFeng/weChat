...
export default {
  ...
  computed: {
    ...mapState(["selectgroupId", "emojis", "gifs", "userInfo", ]),
    ...mapGetters(["selectedgroupChat"])
  },
  methods: {
    initwebsocket: function() {
      ...
      this.webSocket.onmessage = function(evt) {
        if (evt.data.indexOf("}") > -1) {
          ...
          //这里处理接收到的消息
          this.$store.dispatch("recivegroupMessage", JSON.parse(evt.data));
        } else {
          //心跳消息
    ...
    send() {
      ...
      if (this.content.length < 1) {
        ...
        // dispatch含有异步操作，存储
        this.$store.dispatch("sendgroupMessage", msg);
        // 发送完消息清空文本框
				this.content = "";
				//通过 send() 方法来向服务器发送数据
