...
<template>
  <div class="message">
    <header class="header">
      <div class="friendname">{{selectedgroupChat.name}}</div>
    </header>

    <div class="message-wrapper" ref="list">
		
      <ul v-if="selectedgroupChat"> 
			
        <li v-for="item in selectedgroupChat.groupmessages" class="message-item">
          <div class="time">
          ...
          <div class="main" :class="{ self: item.self }">
            <img
              ...
              width="36"
              height="36"
              :src="item.self ? userInfo.avatar : item.avatar"
            />
            <div class="content">
...
export default {
  ...
  computed: {
		...
		...mapGetters(["selectedgroupChat", "groupmessages"]),
		// 当前登录用户和表情
  ...
  watch: {
    // 发送信息后,让信息滚动到最下面
    'selectedgroupChat.groupmessages'() {
	  if(this.$refs.list)
      setTimeout(
        () => (this.$refs.list.scrollTop = this.$refs.list.scrollHeight),
        0
